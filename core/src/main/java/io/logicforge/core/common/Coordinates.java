...
public final class Coordinates implements Iterable<Integer> {
    ...
    /**
     * Since Coordinates objects are immutable and iterating throw parents/children would otherwise create a lot of
     * object churn, we maintain an internal cache. This method returns the canonical version of the Coordinates object
     * representing the supplied coordinate list if it exists. If it does not exist, it will create a new object,
     * cache it for reuse, and return the new object.
    ...
    public Coordinates getSibling(final int siblingIndex) {
        final int size = coordinateList.size();
        return intern(Stream.concat(coordinateList.subList(0, size - 1).stream(), Stream.of(siblingIndex))
                .collect(Collectors.toList()));
    }
    ...
    public Integer getFinalIndex() {
        final int size = coordinateList.size();
        if (size == 0) {
            throw new IllegalStateException("Attempted to find the final segment of the root coordinates");
        }
