...
public class EngineMethodUtil {
    ...
    public static Optional<Pair<EngineMethodType, Object>> analyzeMethod(final Method method) {
        final List<Pair<EngineMethodType, Object>> annotatedTypes = Arrays.stream(EngineMethodType.values())
                .map(methodType ->
                        new Pair<EngineMethodType, Object>(methodType,
        ...
        if (annotatedTypes.size() > 1) {
            ...
            throw new IllegalStateException(
                    String.format(
                            "Method %s has multiple annotations where only one is permitted: %s",
                            method,
                            annotationClassList));
        }
    ...
    public static Optional<EngineMethodType> getTypeForAnnotation(final Object annotation) {
        ...
        return Arrays.stream(EngineMethodType.values())
                .filter(type -> Objects.equals(type, annotationType))
                .findFirst();
    }
}
