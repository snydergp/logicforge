...
class ProcessCompilerTest {
    ...
    @Test
    void compileAndInstantiate_compilesValidBasicProcess() throws IOException, ProcessConstructionException, InterruptedException {
        final String basicSource = FileUtil.loadGeneratedJavaFileSource("basic");
        final List<TypedArgument> constructorArgs = List.of(
                TypedArgument.from(EngineSpec.class, engineSpec),
                TypedArgument.from(ExecutionQueue.class, queue),
                TypedArgument.from(EngineSpecUtils.Functions.class, functions)
        );
        final EngineSpecUtils.TestProcess testProcess =
                compiler.compileAndInstantiate(BASIC_CLASSNAME, basicSource, constructorArgs, EngineSpecUtils.TestProcess.class);
        ...
        final int repetitions = 1000000;
        {
            ...
            for (int i = 0; i < repetitions; i++) {
                doTheThing(Integer.toString(i), i);
            }
            final long end = System.currentTimeMillis();
            System.out.println("Ran %s executions in %s milliseconds".formatted(repetitions, end - start));
        }
    }

    private String doTheThing(final String text, final Integer number) {
        final Integer a = functions.recordPair(functions.concat("Hello, ", text), functions.add(3, number));
        final Integer b = functions.recordPair(functions.concat("Hi, ", text), functions.add(7, number));
