...
public class EngineSpecUtils {
    ...
    /**
     * Builds a process implementing the {@link TestProcess} interface which takes in two parameters (String text and
     * int number) and performs the following actions;
     *
     * <ol>
     *     <li>Record the pair {concatA + text (String), addA + number (Integer)} and store the summed integer
     *     value</li>
     *     <li>Record the pair {concatB + text (String), addB + number (Integer)} and store the summed integer
     *     value</li>
     * </ol>
     *
     * The process then returns to the caller the string <code>"The sum is"</code> followed by the total sum from the two pairs
     *
     ...
     * @return
     */
    public static ProcessConfig<TestProcess> buildBasicProcessConfig(final String concatA, final Integer addA,
                                                        final String concatB, final Integer addB) {
                ...
                .arguments(Map.of(
                        ...
                        List.of(
                                FunctionConfig.builder()
                                        ...
                                        .arguments(
                                                Map.of("a", List.of(
                                                        ReferenceConfig.builder().reference(
                                                                ReferenceConfig.Reference.builder()
                                                                        .coordinateList(List.of(0)).build())
                                                                .build()
                                                ), "b", List.of(
                                                        ReferenceConfig.builder().reference(
                                                                        ReferenceConfig.Reference.builder()
                                                                                .coordinateList(List.of(1)).build())
                                                                .build()
                                                ))
                                        )
                                        .build()
                        ))).build();
        return ProcessConfig.<TestProcess>builder()
                .functionalInterface(TestProcess.class)
                .name("example").rootBlock(blockConfig)
    ...
    private static ExpressionConfig buildConcatConfig(final String a) {
        return FunctionConfig.builder().name("concat").arguments(Map.of("a",
                List.of(ValueConfig.builder().value(a).build()), "b",
                List.of(ReferenceConfig.builder().reference(
                                ReferenceConfig.Reference.builder().coordinateList(ROOT.asList()).path(List.of("text")).build()
                        ).build()
                ))).build();
    }
    ...
    private static ExpressionConfig buildAddConfig(final int a) {
        return FunctionConfig.builder().name("add")
                .arguments(Map.of("a",
                        List.of(ValueConfig.builder().value(Integer.toString(a)).build()),
                        "b",
                        List.of(ReferenceConfig.builder().reference(
                                ReferenceConfig.Reference.builder().coordinateList(ROOT.asList()).path(List.of(
                                        "number")).build()
                        ).build())))
                .build();
    }

    public static EngineSpec buildSpec(final Functions functions) throws EngineConfigurationException {
        return new EngineSpecBuilder()
                .withProviderInstance(functions, true)
